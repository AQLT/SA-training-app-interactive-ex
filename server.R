...
nsa1<-ts(read.table("./Data/exercise1.dat"),start=1996,frequency=12)
nsa2<-ts(read.table("./Data/exercise2.dat"),start=1980,frequency=12)
nsa3<-ts(read.table("./Data/exercise3.dat"),start=1997,frequency=4)
nsa4<-ts(read.table("./Data/exercise4.dat"),start=1987,frequency=4)
...
shinyServer(function(input, output) {
    ...
    output$tsPlot <- renderDygraph({
        ...
        data_graph_dy <- ts.union(datasetInput(),sa1,sa2)
        colnames(data_graph_dy) <- c("NSA","SA1","SA2")
        dygraph(data_graph_dy) %>%
    ...
    output$trendPlot <- renderHighchart({
        ...
        data_graph_hc <- round(ts.union(datasetInput(),sa1,sa2),1)
        colnames(data_graph_hc) <- c("NSA","SA1","SA2")
        
        highchart(type = "stock") %>%
            hc_add_series(data_graph_hc[,"NSA"], name = "NSA") %>%
            hc_add_series(data_graph_hc[,"SA1"], name = "SA1") %>%
            hc_add_series(data_graph_hc[,"SA2"], name = "SA2") %>%
            hc_chart(zoomType = "x") %>% 
    ...
    output$SIPlot1 <- renderPlot({
        ...
        data1$id <- "SA1"
        data2$id <- "SA2"
        data_graph <- rbind(data1,data2)

        ggplot(data_graph, aes(x=year, y=mod))+geom_point(col="blue")+
            geom_point(aes(x=year, y=unmod),col="hotpink", na.rm=TRUE)+
            geom_line(aes(x=year, y=d10),color="red")+
            geom_line(aes(x=year,y=mean),color="darkgreen")+
            facet_wrap(id~period,nrow = 2)+scale_x_continuous()+
            labs(x="",y="Seasonal factor",title="Seasonal factors - adjustment 1 and 2")+
